...
export default function ScrCalendarioTecnologo({ navigation }: any) {
  ...
  const cargarCitasPendientes = async () => {
    try {
      ...
      if (error) {
        ...
        Alert.alert(
          'Error',
          'No se pudieron cargar las citas pendientes. Intenta nuevamente.'
        );
        return;
      }
      ...
      console.error('Error inesperado cargando citas pendientes:', e);
      Alert.alert(
        'Error',
        'No se pudieron cargar las citas pendientes. Intenta nuevamente.'
      );
  ...
  const aceptarCita = async (citaId: string) => {
    try {
      ...
      Alert.alert('Cita aceptada', 'La cita ha sido marcada como confirmada.');
    } catch (e) {
  ...
  const cancelarCita = async (citaId: string) => {
    try {
      ...
      // Al estar cancelada, desaparece de la bandeja y el horario queda libre
      setCitasPendientes(prev => prev.filter(c => c.id !== citaId));
      Alert.alert(
        'Cita cancelada',
      ...
      <ScrollView contentContainerStyle={styles.main}>
        ...
        {citasPendientes.length === 0 ? (
          ...
          citasPendientes.map(cita => {
            ...
            const fechaBonita = new Date(cita.fecha).toLocaleDateString(
              'es-CL',
              {
                weekday: 'long',
                year: 'numeric',
